(function(win,doc){ // EC.EUROPA.EU 2013 "RESPONSIVE DESIGN" - 2.5.1
...
var _$ = {
	...
	ready       :function(func){if(_$.domIsReady){func();return;}if(!_$.loadEvents){_$.loadEvents=[];}var doc=document;function isReady(){_$.domIsReady=true;clearInterval(_$.loadTimer);while(_$.exec=_$.loadEvents.shift()){_$.exec();}if(_$.ieReady){_$.ieReady.onreadystatechange='';}}if(!_$.loadEvents[0]){if(doc.addEventListener){doc.addEventListener("DOMContentLoaded",isReady,false);}else if(isIE < 9){var d=win.doc,done=!1;!function(){try{d.documentElement.doScroll("left")}catch(e){return void setTimeout(arguments.callee,50)}isReady()}(),d.onreadystatechange=function(){"complete"==d.readyState&&(d.onreadystatechange=null,isReady())}}else if(/WebKit|KHTML|iCab/i.test(navigator.userAgent)){_$.loadTimer=setInterval(function(){if(/loaded|complete/.test(doc.readyState)){isReady();}},10);}_$.oldOnload=window.onload;window.onload=function(){isReady();if(_$.oldOnload){_$.oldOnload();}};}_$.loadEvents.push(func);},
...
// CORPORATE
	...
	mobileRun	:function(){
		...
		// accessibility menu
		if(am){
			am.innerHTML=(!sf)?am.innerHTML:am.innerHTML+'<li class="m-link m-form"><a href="#searchform" onclick="return components.mobile.search(this)"><span>'+doc.getElementsByTagName("label")[0].innerHTML+'</span></a></li>';
