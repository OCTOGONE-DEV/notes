  ...
  $wt.piwik = {
    ...
    run: function(j) {
      ...
      if (!j["siteID"]) {
        ...
        // Check and add the ec_departments dimension to the _paq, if any
        $wt.piwik.setDepartmentsDimension();
    ...
    setDepartmentsDimension: function() {
      var maxSize = 253, // 255 - inicial dot - last dot
          strSize = 0;

      try {
        var A = [];
        [].forEach.call( document.querySelector("meta[name='ec_departments']").getAttribute('content').split(','), function(e) {
          var ckeyword = e.trim().toUpperCase();
          
          strSize += ckeyword.length + 1; // 1 is the added dot
          if (strSize > maxSize) {
            return false;
          }
          A.push( ckeyword );
        ...
        A.sort();
        // It will be turned on in After 30 of August 2018.
        // _paq.push(['setCustomDimension', 3, '.' + (A.join('.')) + '.']);
     } catch(e) {
      return e;
     }
    }
